    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['EpidemSys:secondary:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['EpidemSys:secondary:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['EpidemSys:secondary:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['EpidemSys:secondary:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="secondaryList" @selection-change="handleSelectionChange">
...
<script>
import { listSecondary, getSecondary, delSecondary, addSecondary, updateSecondary } from "@/api/EpidemSys/secondary"
...
export default {
  ...
  methods: {
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map(item => item.secId)
    ...
    handleAdd() {
      this.reset()
      this.open = true
      this.title = "添加二级亲属过敏史"
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset()
      const secId = row.secId || this.ids
      getSecondary(secId).then(response => {
        this.form = response.data
        this.open = true
        this.title = "修改二级亲属过敏史"
      })
    },
    /** 提交按钮 */
    submitForm() {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const secIds = row.secId || this.ids
      this.$modal.confirm('是否确认删除二级亲属过敏史编号为"' + secIds + '"的数据项？').then(function() {
        return delSecondary(secIds)
      }).then(() => {
        this.getList()
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('EpidemSys/secondary/export', {
        ...this.queryParams
