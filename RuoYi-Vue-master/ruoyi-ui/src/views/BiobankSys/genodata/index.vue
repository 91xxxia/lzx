  ...
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['BiobankSys:genodata:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['BiobankSys:genodata:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['BiobankSys:genodata:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['BiobankSys:genodata:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="genodataList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="iL4基因型" align="center" prop="il4Genotype" />
      <el-table-column label="iL3基因型" align="center" prop="il13Genotype" />
      <el-table-column label="丝聚蛋白基因突变" align="center" prop="filaggrinMutation" />
      <el-table-column label="TSLP基因突变" align="center" prop="tslpVariant" />
      <el-table-column label="其他SNP" align="center" prop="otherSnpData" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="iL4基因型" prop="il4Genotype">
          <el-input v-model="form.il4Genotype" placeholder="请输入iL4基因型" />
        </el-form-item>
        <el-form-item label="iL3基因型" prop="il13Genotype">
          <el-input v-model="form.il13Genotype" placeholder="请输入iL3基因型" />
        </el-form-item>
        <el-form-item label="丝聚蛋白基因突变" prop="filaggrinMutation">
          <el-input v-model="form.filaggrinMutation" placeholder="请输入丝聚蛋白基因突变" />
        </el-form-item>
        <el-form-item label="TSLP基因突变" prop="tslpVariant">
          <el-input v-model="form.tslpVariant" placeholder="请输入TSLP基因突变" />
        </el-form-item>
        <el-form-item label="其他SNP" prop="otherSnpData">
          <el-input v-model="form.otherSnpData" type="textarea" placeholder="请输入内容" />
        </el-form-item>
...
<script>
import { listGenodata, getGenodata, delGenodata, addGenodata, updateGenodata } from "@/api/BiobankSys/genodata"
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true
      listGenodata(this.queryParams).then(response => {
        this.genodataList = response.rows
    ...
    handleUpdate(row) {
      ...
      getGenodata(genomicId).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.genomicId != null) {
            updateGenodata(this.form).then(response => {
              this.$modal.msgSuccess("修改成功")
              this.open = false
              this.getList()
            })
          } else {
            addGenodata(this.form).then(response => {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('BiobankSys/genodata/export', {
        ...this.queryParams
      }, `genodata_${new Date().getTime()}.xlsx`)
    }
  }
}
</script>
