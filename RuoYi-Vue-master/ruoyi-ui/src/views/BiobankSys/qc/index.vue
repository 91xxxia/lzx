<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="qcList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="质量控制号" align="center" prop="qcId" />
      <el-table-column label="样本号" align="center" prop="specimenId" />
      <el-table-column label="质量控制日期" align="center" prop="qcDate" width="180">
        <template slot-scope="scope">
          <span>{{ parseTime(scope.row.qcDate, '{y}-{m}-{d}') }}</span>
        </template>
      </el-table-column>
      <el-table-column label="DNA浓度" align="center" prop="dnaConcentration" />
      <el-table-column label="DNA质量比" align="center" prop="dnaQualityRatio" />
      <el-table-column label="RNA完整性" align="center" prop="rnaIntegrityNumber" />
      ...
      <el-table-column label="质控操作" align="center" prop="qcOperator" />
      <el-table-column label="质控方法" align="center" prop="qcMethod" />
      <el-table-column label="质控注意事项" align="center" prop="qcNotes" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="质量控制日期" prop="qcDate">
          <el-date-picker clearable
            v-model="form.qcDate"
            type="date"
            value-format="yyyy-MM-dd"
            placeholder="请选择质量控制日期">
          </el-date-picker>
        </el-form-item>
        <el-form-item label="DNA浓度" prop="dnaConcentration">
          <el-input v-model="form.dnaConcentration" placeholder="请输入DNA浓度" />
        </el-form-item>
        <el-form-item label="DNA质量比" prop="dnaQualityRatio">
          <el-input v-model="form.dnaQualityRatio" placeholder="请输入DNA质量比" />
        </el-form-item>
        <el-form-item label="RNA完整性" prop="rnaIntegrityNumber">
          <el-input v-model="form.rnaIntegrityNumber" placeholder="请输入RNA完整性" />
        </el-form-item>
        <el-form-item label="蛋白质浓度" prop="proteinConcentration">
          <el-input v-model="form.proteinConcentration" placeholder="请输入蛋白质浓度" />
        </el-form-item>
        <el-form-item label="溶血水平" prop="hemolysisLevel">
          <el-input v-model="form.hemolysisLevel" placeholder="请输入溶血水平" />
        </el-form-item>
        <el-form-item label="污染状态" prop="contaminationStatus">
          <el-input v-model="form.contaminationStatus" placeholder="请输入污染状态" />
        </el-form-item>
        <el-form-item label="质控操作" prop="qcOperator">
          <el-input v-model="form.qcOperator" placeholder="请输入质控操作" />
        </el-form-item>
        <el-form-item label="质控方法" prop="qcMethod">
          <el-input v-model="form.qcMethod" placeholder="请输入质控方法" />
        </el-form-item>
        <el-form-item label="质控注意事项" prop="qcNotes">
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        qcDate: [
          { required: true, message: "质量控制日期不能为空", trigger: "blur" }
        ],
      }
    }
  },
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除样本质量控制编号为"' + qcIds + '"的数据项？').then(function() {
        return delQc(qcIds)
      }).then(() => {
        this.getList()
