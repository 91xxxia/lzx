<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="storageList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="冰箱编号" align="center" prop="storageLocation" />
      ...
      <el-table-column label="冰冻日期" align="center" prop="freezeDate" width="180">
        <template slot-scope="scope">
          <span>{{ parseTime(scope.row.freezeDate, '{y}-{m}-{d}') }}</span>
        </template>
      </el-table-column>
      <el-table-column label="冰冻方法" align="center" prop="freezeMethod" />
      <el-table-column label="冻融循环" align="center" prop="freezeThawCycles" />
      <el-table-column label="存储状态" align="center" prop="storageStatus" />
      <el-table-column label="最后检查日期" align="center" prop="lastCheckDate" width="180">
        <template slot-scope="scope">
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="冰箱编号" prop="storageLocation">
          <el-input v-model="form.storageLocation" placeholder="请输入冰箱编号" />
        </el-form-item>
        ...
        <el-form-item label="冰冻日期" prop="freezeDate">
        ...
        <el-form-item label="冰冻方法" prop="freezeMethod">
          <el-input v-model="form.freezeMethod" placeholder="请输入冰冻方法" />
        </el-form-item>
        ...
        <el-form-item label="存储状态" prop="storageStatus">
          <el-input v-model="form.storageStatus" placeholder="请输入存储状态" />
        </el-form-item>
        <el-form-item label="最后检查日期" prop="lastCheckDate">
          <el-date-picker clearable
            v-model="form.lastCheckDate"
            type="date"
            value-format="yyyy-MM-dd"
            placeholder="请选择最后检查日期">
          </el-date-picker>
        </el-form-item>
      </el-form>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        storageLocation: [
          { required: true, message: "冰箱编号不能为空", trigger: "blur" }
        ],
        storageTemperature: [
          { required: true, message: "存储温度不能为空", trigger: "blur" }
        ],
        freezeDate: [
          { required: true, message: "冰冻日期不能为空", trigger: "blur" }
        ],
      }
    }
  },
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除样本存储编号为"' + storageIds + '"的数据项？').then(function() {
